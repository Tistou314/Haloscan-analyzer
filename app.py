...
# =============================================================================
# SIDEBAR — Chargement des fichiers (rien ne se lance automatiquement)
# =============================================================================
...
with st.sidebar:
    ...
    uploaded_file_p2 = st.file_uploader("2️⃣ CSV Haloscan Période 2 (optionnel)", type=['csv'], key="haloscan_p2")
    ...
    # --- Résumé fichiers ---
    st.divider()
    nb_files = sum([bool(uploaded_file_p1), bool(uploaded_file_p2), bool(uploaded_leads), bool(uploaded_gsc)])
    if nb_files > 0:
        st.markdown(f"**{nb_files} fichier(s) prêt(s)**")
        if uploaded_file_p1: st.caption(f"✅ P1 : {uploaded_file_p1.name}")
        if uploaded_file_p2: st.caption(f"✅ P2 : {uploaded_file_p2.name}")
        if uploaded_leads: st.caption(f"✅ Leads : {uploaded_leads.name}")
        if uploaded_gsc: st.caption(f"✅ GSC : {uploaded_gsc.name}")
    
    # --- BOUTON ---
    st.divider()
    can_launch = uploaded_file_p1 is not None
    if uploaded_file_p1 and uploaded_file_p2:
        st.caption("📊 Mode : Analyse comparative (2 périodes)")
    elif uploaded_file_p1:
        st.caption("📊 Mode : Analyse simple (1 période)")
    launch = st.button("🚀 Lancer l'analyse", type="primary", disabled=not can_launch, use_container_width=True)
...
def label_to_month(label):
    mm = {'jan':'01','fev':'02','fév':'02','mar':'03','avr':'04','apr':'04','mai':'05','may':'05',
          'jun':'06','jui':'07','jul':'07','aou':'08','aoû':'08','aug':'08','sep':'09','oct':'10',
...
if not st.session_state.get('analysis_launched', False):
    st.markdown("---")
    if not uploaded_file_p1:
        st.info("👆 **Chargez vos fichiers dans la sidebar** puis cliquez sur **🚀 Lancer l'analyse**")
    else:
        st.success("**Fichiers chargés.** Cliquez sur **🚀 Lancer l'analyse** dans la sidebar quand vous êtes prêt.")
    st.stop()

# =============================================================================
# TRAITEMENT (uniquement après clic sur le bouton)
# =============================================================================
...
with st.spinner("📊 Chargement et traitement des données..."):
    ...
    if uploaded_leads:
        try:
            ...
            if mc_chk:
                ...
                if mv > 500:
                    st.sidebar.error(f"⚠️ Moyenne={mv:.0f} — Probablement les VISITES")
                else:
                    st.sidebar.info(f"✅ Moyenne={mv:.1f} — Données leads OK")
        except Exception:
        ...
        for c in month_cols:
            if c in leads_df.columns:
                leads_df[c] = pd.to_numeric(leads_df[c], errors='coerce').fillna(0)
    ...
    if has_dual_haloscan:
        ...
        for req, lab in [('mot_cle', 'Mot-clé'), ('url', 'URL')]:
            if req not in df_p1.columns:
                st.error(f"❌ Colonne '{req}' non trouvée dans P1. Colonnes : {list(df_p1.columns)}")
                st.stop()
            if req not in df_p2.columns:
                st.error(f"❌ Colonne '{req}' non trouvée dans P2. Colonnes : {list(df_p2.columns)}")
                st.stop()
        ...
        c_dp1 = 'pos_debut_p1' if 'pos_debut_p1' in df.columns else None
        c_fp1 = 'pos_fin_p1' if 'pos_fin_p1' in df.columns else None
        c_fp2 = 'pos_fin_p2' if 'pos_fin_p2' in df.columns else None
        c_dp2 = 'pos_debut_p2' if 'pos_debut_p2' in df.columns else None
        df['ancienne_pos'] = (df[c_dp1] if c_dp1 else pd.Series(0, index=df.index)).fillna(df[c_dp2] if c_dp2 else 0)
        df['derniere_pos'] = (df[c_fp2] if c_fp2 else pd.Series(0, index=df.index)).fillna(df[c_fp1] if c_fp1 else 0)
...
if st.session_state.get('analysis_launched', False) and 'df' in dir():
    ...
    if has_leads and 'url' in df.columns:
        ...
        def tl(row):
            e = row.get('leads_evolution', 0) or 0
            p = row.get('leads_evolution_pct', 0) or 0
        ...
        for c in ['leads_total', 'leads_avant', 'leads_apres', 'leads_evolution']:
            df[c] = 0
    ...
    with st.sidebar:
        with st.expander("🔍 Colonnes", expanded=False):
    ...
    with st.sidebar:
        ...
        vol_range = st.slider("Volume", int(df['volume'].min() or 0), int(df['volume'].max() or 10000),
                              (int(df['volume'].min() or 0), int(df['volume'].max() or 10000))) if 'volume' in df.columns else None
...
elif st.session_state.get('analysis_launched', False):
    st.error("❌ Aucun fichier Haloscan chargé. Uploadez au moins un CSV puis relancez.")
